#!/usr/bin/env python3
import lib_biased_mnist
from pathlib import Path
import os, sys, inspect

currentdir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
parentdir = os.path.dirname(currentdir)
sys.path.insert(0, currentdir)
sys.path.insert(0, parentdir)


from lib_experiment import Experiment

LABEL_CORRELATIONS = [0.999, 0.99, 0.9]
NOISE_LEVELS = [0]

experiment = Experiment(
    name=Path(__file__).stem,
    problem="biased_mnist",
    n_runs=3,
    f_pre_experiment=lambda: lib_biased_mnist.regenerate_all_data(
        LABEL_CORRELATIONS, NOISE_LEVELS
    ),
    experiment_dict={
        "diversity_loss.independence_measure": ["cka"],
        "diversity_loss.kernel": ["rbf"],
        "compute_combined_loss.diversity_loss_coefficient": [
            0,
            1 / 32,
            1 / 4,
            1 / 2,
            1,
            2,
            4,
            8,
            16,
            32,
            64,
            128,
            256,
            512,
        ],
        "BiasedMnistProblem.training_data_label_correlation": LABEL_CORRELATIONS,
        "BiasedMnistProblem.filter_for_digits": list(range(10)),
        "BiasedMnistProblem.model_type": "mlp",
        "BiasedMnistProblem.background_noise_level": NOISE_LEVELS,
        "Problem.n_epochs": 100,
        "Problem.initial_lr": [0.001, 0.01],
        "Problem.decrease_lr_at_epochs": [20, 40, 80],
        "Problem.n_models": [2, 3, 4, 5],
        "Problem.optimizer": "@tf.keras.optimizers.Nadam",
        "tf.keras.optimizers.Nadam.epsilon": 0.001,
        "get_weight_regularizer.strength": 0.01,
    },
)

